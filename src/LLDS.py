...
class LLDS:
    ...
    def smooth(self, kmeans, kcovars, us):
        ...
        for t in range(cols-2, 0, -1):
            Pt = self.A @ kcovars[:, :, t] @ numpy.transpose(self.A) + self.Q
            Jt = kcovars[:, :, t] @ numpy.transpose(self.A) @ numpy.linalg.inv(Pt)
            smoothedmeans[:, t] = Jt @ (smoothedmeans[:, t+1] - self.A @ kmeans[:, t] - self.B*us[:, t-1])
            smoothedmeans[:, t] += kmeans[:, t]
        ...
        smoothedmeans[:, 0] = kmeans[:, 0] + Jt @ (smoothedmeans[:, 1] - self.A @ kmeans[:, 0])
