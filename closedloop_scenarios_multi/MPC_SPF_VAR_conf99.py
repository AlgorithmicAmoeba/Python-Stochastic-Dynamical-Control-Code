...
lin_models[0] = RBPF.Model(linsystems[opoint].A, linsystems[opoint].B, linsystems[opoint].b, params.C2,
                           params.Q, params.R2)
...
params.us[0] = MPC.mpc_var(params.spfmeans[:, 0] - lin_models[ind].b, params.spfcovars[:, :, 0], horizon,
                           lin_models[ind].A, numpy.matrix(lin_models[ind].B), lin_models[ind].b,
                           aline, bline, cline, params.QQ, params.RR, yspfix, usps[ind], 15000.0,
                           1000.0, params.Q, 9.21, True)  # get the controller input
...
for t in range(1, params.N):
    ...
    if t % 1 == 0:
        ...
        params.us[t] = MPC.mpc_var(params.spfmeans[:, t] - lin_models[ind].b, params.spfcovars[:, :, t],
                                   ...
                                   usps[ind], 15000.0, 1000.0, params.Q, 9.21, True)  # get the controller input
    else:
