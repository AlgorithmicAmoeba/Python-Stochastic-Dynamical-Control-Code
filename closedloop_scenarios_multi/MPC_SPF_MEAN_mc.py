...
while mciter < mcN-1:
    ...
    ydists = numpy.array([scipy.stats.multivariate_normal(cov=params.R2),
    ...
    for t in range(1, params.N):
        ...
        if t % 1 == 0:
            ...
            params.us[t] = MPC.mpc_mean(params.spfmeans[:, t] - lin_models[ind].b, horizon, lin_models[ind].A,
                                        numpy.matrix(lin_models[ind].B), lin_models[ind].b, aline, bline, cline,
                                        params.QQ, params.RR, yspfix, usps[ind], 15000.0, 1000.0)
        else:
...
numpy.savetxt("spf_mean.csv", filteredResults, delimiter=",", fmt="%f")
numpy.savetxt("spf_mean_mc2.csv", xconcen, delimiter=",", fmt="%f")
