...
params.us[0] = MPC.mpc_var(params.spfmeans[:, 0] - lin_models[ind].b, params.spfcovars[:, :, 0], horizon, lin_models[ind].A, numpy.matrix(lin_models[ind].B), lin_models[ind].b, aline, bline, cline, params.QQ, params.RR, yspfix, usps[ind], 15000.0, 1000.0, params.Q, 4.6052, True)  # get the controller input
...
for t in range(1, params.N):
    ...
    if t % 1 == 0:
        ...
        yspfix = setpoint - lin_models[ind].b
        params.us[t] = MPC.mpc_var(params.spfmeans[:, t] - lin_models[ind].b, params.spfcovars[:, :, t], horizon, lin_models[ind].A, numpy.matrix(lin_models[ind].B), lin_models[ind].b, aline, bline, cline, params.QQ, params.RR, yspfix, usps[ind], 15000.0, 1000.0, params.Q, 4.6052, True)  # get the controller input
    else:
...
Results.plot_ellipses2(params.ts, params.xs, params.spfmeans, params.spfcovars, [aline, cline], [linsystems[1].op[1], 422.6], True, 4.6052, 1, "best")
