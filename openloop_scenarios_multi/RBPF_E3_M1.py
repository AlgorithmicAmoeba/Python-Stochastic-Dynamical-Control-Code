...
RBPF.init_filter(particles, 0.0, params.ys1[1], models)
...
for k in range(len(linsystems)):
    loc = numpy.where(particles.ss == k)[0][0]
    switchtrack[k, 1] = sum(particles.ws[loc])
...
for t in range(1, params.N):
    ...
    for k in range(len(linsystems)):
        loc = numpy.where(particles.ss == k)[0][0]
        switchtrack[k, t] = sum(particles.ws[loc])
